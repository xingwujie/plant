...
const logger = require('../logging/logger').create('lib.render.plant');
...
const target = (req, res) => {
  ...
  return singlePlant(user, plantId, (singlePlantErr, initialState = {}) => {
    if (singlePlantErr) {
      ...
      }).catch((renderToStringError) => {
        logger.error('render.plant.singlePlant renderToString Error', { renderToStringError });
      })
      ;
    }
  });
