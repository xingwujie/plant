...
module.exports = (app) => {
  ...
  app.post('/api/plant', requireToken, (req, res) => {
    ...
    validatePlant(plant, { isNew }, (validatePlantErr, transformed) => {
      ...
      return mongo.createPlant(transformed, req.user._id, (createPlantErr, result) => {
        if (createPlantErr) {
  ...
  app.get('/api/plant/:plantId', (req, res) => {
    ...
    return mongo.getPlantById(plantId, loggedInUserId, (err, result) => {
  ...
  app.put('/api/plant', requireToken, (req, res) => {
    ...
    validatePlant(plant, { isNew }, (err, transformed) => {
      ...
      const loggedInUserId = req.user && req.user._id;
      return mongo.updatePlant(transformed, loggedInUserId, (updatePlantErr, result) => {
        if (updatePlantErr) {
  ...
  app.delete('/api/plant/:plantId', requireToken, (req, res) => {
    ...
    return mongo.deletePlant(plantId, userId, (plantDeleteErr, deletedCount) => {
      if (plantDeleteErr) {
