...
export default (app) => {
  ...
  function imageNote(req, res, next) {
    logger.trace('imageNote called');
    // TODO: Update Note document with image meta data
    return next();
  }

  app.post('/api/upload', requireToken, imageNote, upload.array('photos', 3), (req, res) => {
    logger.trace('Successfully uploaded ' + req.files.length + ' files!');
    return res.status(200).send({success: true});
  });
