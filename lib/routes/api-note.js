...
export default (app) => {
  ...
  app.post('/api/note', requireToken, (req, res) => {
    ...
    debug('/api/note req.user._id:', req.user._id);
    const isNew = true;
    ...
    validateNote(req.body, {isNew}, (err, transformed) => {
      ...
      debug('/api/note transformed:', transformed);
      mongo.createNote(transformed, (err2, result) => {
        if(err) {
  ...
  app.put('/api/note', requireToken, (req, res) => {
    ...
    validateNote(req.body, {isNew}, (err, transformed) => {
      ...
      mongo.updateNote(transformed, (err2, result) => {
        if(err2) {
  ...
  app.delete('/api/note/:noteId', requireToken, (req, res) => {
    ...
    mongo.deleteNote(noteId, userId, (err, result) => {
      if(err) {
