...
export default (app) => {
  ...
  function makeS3KeyFromFile(file) {
    ...
    return `${firstDirectory}/orig/${filename}`;
  }

  function makeS3KeyFromImage(image) {
    return `${firstDirectory}/orig/${image.id}${image.ext && '.'}${image.ext}`;
  }
    ...
    function deleteImages(note, cb) {
      if(note && note.images && note.images.length) {
        ...
        const options = {
          ...
          Delete: {
            Objects: note.images.map(image => {
              return {Key: makeS3KeyFromImage(image)};
            })
          }
        };
  ...
  */
  ...
  function key(req, file, cb) {
    cb(null, makeS3KeyFromFile(file));
  }
  ...
  function imageNote(req, res, next) {
    ...
    note.images = (note.images || []).concat(req.files.map(file => {
      ...
      if(parts.length !== 2) {
        logger.error('File name (key) in imageNote is in wrong format', {file});
      }
      const [fileKey, ext] = parts;
      const keyParts = fileKey.split('/');
      const id = keyParts[keyParts.length - 1];
      let {originalname: original, size} = file;
  ...
  app.put('/api/image-complete', (req, res) => {
    ...
    logger.info('/api/image-complete body', {body: req.body});
