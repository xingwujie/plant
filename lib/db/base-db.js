    ...
    this.getDb((err, db) => {
      ...
      db.get(obj._id, (error, existing) => {
        ...
        if(existing.userId !== userId) {
          debug(`updateByUser failed because document belongs to ${existing.userId} and userId param is ${userId}`);
          return cb({statusCode: 403, message: `userId mismatch in update`});
        }

        _.assign(obj, _.pick(existing, ['_rev', 'userId', 'type']));
