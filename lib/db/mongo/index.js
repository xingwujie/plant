  ...
  getAllUsers(cb) {
    this.GetDb((err, db) => {
      ...
      read(db, 'user', userQuery, userFields, userOptions, (getUserError, users) => {
        ...
        const locationQuery = {};
        const locationFields = {_id: true, ownerIds: true, managerIds: true};
        ...
        read(db, 'location', locationQuery, locationFields, locationOptions, (locationReadError, locations) => {
          ...
          const stringlocations = (locations || []).map(location => {
            const userIds = (location.ownerIds || [])
          ...
          const usersWithlocationIds = (users || []).map(user => {
            user.locationIds = stringlocations.filter(location => location.userIds.includes(user._id.toString())).map(p => p._id);
            return user;
          });
