...
class MongoDb {
  ...
  /**
   * Rebase the plant's location by the user's location
  ...
  rebasePlantByLoc(plant, cb) {
    if(userLocCache[plant.userId]) {
      return cb(null, this.rebasePlant(plant, userLocCache[plant.userId]));
    } else {
      this.GetDb((err, db) => {
        const userQuery = {
        ...
        read(db, 'user', userQuery, fields, options, (readUserError, users) => {
          ...
          if(readUserError) {
            logger.error('rebasePlantByLoc readUserError:', {readUserError, userQuery});
            delete plant.loc;
            return cb(readUserError, plant);
          } else {
            const user = users[0];
            if(user.loc) {
              userLocCache[plant.userId] = user.loc;
              return cb(null, this.rebasePlant(plant, userLocCache[plant.userId]));
            } else {
              userLocCache[plant.userId] = plant.loc;
              user.loc = plant.loc;

              Update.updateOne(db, 'user', userQuery, user, (updateUserError) => {
                return cb(updateUserError, this.rebasePlant(plant, userLocCache[plant.userId]));
              });
            }
          }
        });
  ...
  convertLocationDataTypesForSaving(loc) {
    ...
    if(loc.userIds && loc.userIds.length) {
      loc.userIds = loc.userIds.map(plantId => new ObjectID(plantId));
    }
  }
  ...
  convertLocationDataForRead(loc) {
    ...
    if(_.isArray(loc)) {
      ...
      if(convertedLocation.plantIds && convertedLocation.plantIds.length) {
        convertedLocation.plantIds = convertedLocation.plantIds.map(plantId => plantId.toString());
      }
  ...
  createLocation(loc, cb) {
    this.GetDb((err, db) => {
      if(!loc.userIds) {
        const errMsg = 'userIds must be specified as part of location when creating a location';
  ...
  getLocationssByUserId(userId, cb) {
    const query = {userIds: new ObjectID(userId)};
    this.getLocationsByQuery(query, cb);
  }
  ...
  updateLocationById(loc, cb) {
    this.GetDb((err, db) => {
      if(!loc.userIds) {
        const errMsg = 'userIds must be specified as part of location when updating a location';
        logger.error(errMsg, {loc});
        return cb(errMsg);
      }
      this.convertLocationDataTypesForSaving(loc);
      const query = _.pick(loc, ['_id']);
      const set = _.omit(loc, ['_id']);
