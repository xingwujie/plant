...
class MongoDb {
    ...
    const createUser = (data, done) => {
      if (data.user) {
        ...
        Create.createOne(data.db, 'user', userDetails, (createOneError, createdUser) => {
          ...
          this.createLocation(location, (createLocationError, createdLocation) => {
  ...
  getUsersWithLocations(users, locations) {
    return (users || []).map((user) => {
      const filteredLoc = (locations || []).filter(location => location.userIds.find(userLoc => userLoc.id === user._id));
  ...
  getAllUsers(cb) {
    this.GetDb((err, db) => {
      ...
      read(db, 'user', userQuery, userFields, userOptions, (getUserError, users) => {
        ...
        read(db, 'location', locationQuery, locationFields, locationOptions, (locationReadError, locations) => {
  ...
  rebasePlantByLoc(plant, cb) {
    ...
    this.GetDb((err, db) => {
      ...
      read(db, 'location', locationQuery, fields, options, (readLocationError, locations) => {
        ...
        Update.updateOne(db, 'location', locationQuery, locat, updateLocationError => cb(updateLocationError, this.rebasePlant(plant, locationLocCache[plant.locationId])));
      });
    });
  }
  ...
  updatePlant(plant, loggedInUserId, cb) {
    ...
    this.GetDb((err, db) => {
      ...
      Update.updateOne(db, 'plant', query, set, (updatePlantError) => {
        this.convertPlantDataForRead(plant, loggedInUserId, (convertPlantDataForReadErr, convertedPlant) => cb(updatePlantError, convertedPlant));
      });
    });
  }
  ...
  updateNote(note, cb) {
    this.GetDb((err, db) => {
      ...
      Update.updateOne(db, 'note', query, set, updateNoteError =>
      ...
      read(db, 'location', query, locationFields, locationOptions, (locationReadError, locationsData) => {
        ...
        read(db, 'plant', plantQuery, plantFields, plantOptions, (plantReadError, plants) => {
          ...
          const locationsWithPlantIds = (locations || []).map((location) => {
            location.plantIds = stringPlants.filter(plant => plant.locationId === location._id.toString()).map(p => p._id);
            return location;
          });
  ...
  updateLocationById(loc, cb) {
    this.GetDb((err, db) => {
      ...
      Update.updateOne(db, 'location', query, set, updateLocationError =>
        // results => {n:1, nModified:1, ok:1}
