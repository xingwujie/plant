...
class MongoDb {
  ...
  getLocationsByUserId(userId, cb) {
    ...
    this.getLocationsByQuery(query, (locationsByQueryError, locations) => {
      // A location object looks like this:
      // { _id, userId, userIds: [{ id, role }], title, loc: {type, coordinates: {}}}

      // Get userIds for all locations
      const userIds = (locations || []).reduce((acc, location) => {
        const ids = (location.userIds || []).map(user => user.id.toString());
        return acc.concat(ids);
      }, []);
      this.getUserNameByIds(userIds, (getUserNameError, usersByIds = []) => {
        if (getUserNameError) {
          return cb(getUserNameError);
        }
        if (!usersByIds || !usersByIds.length) {
          logger.error(`Unexpected empty usersByIds collection after query for these userIds ${userIds.join()}`);
        }
        const userNameMap = usersByIds.reduce((acc, user) => {
          acc[user._id.toString()] = user.name;
          return acc;
        }, {});
        const uiLocations = locations.map((location) => {
          const users = location.userIds.map(uid => ({
            _id: uid.id,
            name: userNameMap[uid.id],
            role: uid.role,
          }));
          return {
            _id: location._id,
            title: location.title,
            role: location.userIds.find(uid => uid.id === userId).role,
            users,
          };
        });
        return cb(null, uiLocations);
      });
    });
  }
