...
class MongoDb {
  ...
  getNotesLatest(qty, cb) {
    ...
    this.GetDb((err, db) => {
      db.collection('note').find().sort({date: -1}).limit(qty).toArray( (noteErr, notes) => {
        if(noteErr) return cb(noteErr);

        this.getNotesWithPlants(notes, cb); // next step: match up with plant names
      });
  ...
  getNotesWithPlants(notes, cb){

    // Grab the plant ids off each note (can be more than 1 per note)
    let plant_ids = [];
    notes.map( ({ plantIds }) => {
      plantIds.map((id) => { plant_ids.push(id); });
    });

    this.getPlantsByIds(plant_ids, null, (err, plants) => {
      if(err) return cb(err);

      // Map over the notes and add the plant info to them
      notes.map( (curr_note) => {
        curr_note.plants = plants.filter( (curr_plant) => {

		  // Cast this to a string otherwise it will fail, str to obj comparison
		  let all_ids = curr_note.plantIds.toString();

		  return all_ids.indexOf(curr_plant._id) != -1;
		});
      });
