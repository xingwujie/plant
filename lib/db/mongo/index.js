...
class MongoDb {
  ...
  getAllUsers(cb) {
    this.GetDb((err, db) => {
      ...
      read(db, 'user', query, fields, options, cb);
    });
  }
  ...
  /**
   * Until all the dates in the DB are converted to Integers this might have to
   * convert date types or integers
   * @param {Date|Integer} isoDate - a date to convert
   * @returns {Integer} - The date as an integer
   */
  extractDateOnly(isoDate) {
    // logger.trace('extractDateOnly isoDate', {'typeof': typeof isoDate, isoDateString: '' + isoDate, keys: Object.keys(isoDate), isoDate: isoDate.toISOString().slice(0, 10) });
    return _.isDate(isoDate)
      ? utils.dateToInt(isoDate)
      : isoDate;
  }

  convertPlantDataForRead(plant) {
    if(!plant) {
      return plant;
    }
    if(_.isArray(plant)) {
      return plant.map(this.convertPlantDataForRead, this);
    } else {
      ...
      if(convertedPlant.plantedDate) {
        convertedPlant.plantedDate = this.extractDateOnly(convertedPlant.plantedDate);
      }
      if(convertedPlant.purchasedDate) {
        convertedPlant.purchasedDate = this.extractDateOnly(convertedPlant.purchasedDate);
      }
  ...
  createPlant(plant, cb) {
    this.GetDb((err, db) => {
      ...
      Create.createOne(db, 'plant', plant, (createOnePlantErr, createdPlant) => {
        ...
        return cb(null, this.convertPlantDataForRead(createdPlant));
      });
    });
  }
    ...
    this.GetDb((err, db) => {
      ...
      read(db, 'plant', query, fields, options, (readError, plants) => {
        if(readError) {
          ...
          read(db, 'note', query, {_id: true}, {}, (noteReadError, notes) => {
            if(noteReadError) {
              logger.error('note noteReadError', {noteReadError});
              return cb(noteReadError);
            }
            // Convert Mongo ObjectIds to strings
            const plant = this.convertPlantDataForRead(plants[0]);
        ...
        if(user && user.length === 1) {
          read(db, 'plant', { userId }, fields, options, (readPlantError, plants) => {
            if(readPlantError) {
              ...
              return cb(null, this.convertPlantDataForRead(plants || []));
            }
          });
        } else {
  ...
  updatePlant(plant, cb) {
    ...
    this.GetDb((err, db) => {
      ...
      Update.updateOne(db, 'plant', query, set, (updatePlantError, updatedPlant) => {
        return cb(updatePlantError, this.convertPlantDataForRead(updatedPlant));
      });
    });
  }
  ...
  convertNoteDataForRead(note) {
    ...
    if(_.isArray(note)) {
      ...
      if(convertedNote.userId) {
        ...
        logger.error('In convertNoteDataForRead() there is no userId', {convertedNote});
      }
      if(convertedNote.date) {
        convertedNote.date = this.extractDateOnly(convertedNote.date);
      }
  ...
  updateNote(note, cb) {
    this.GetDb((err, db) => {
      ...
      Update.updateOne(db, 'note', query, set, (updateNoteError, updatedNote) => {
        return cb(updateNoteError, this.convertNoteDataForRead(updatedNote));
      });
    });
  }
  ...
  // Temporary methods for DB update
