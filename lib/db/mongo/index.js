      ...
      if(getUserByQueryError) {
        ...
        this.GetDb((err, db) => {
          read(db, 'location', locationQuery, {_id: true, userIds: true}, {}, (locationReadError, locations) => {
            ...
            user.locationIds = (locations || []).map(location => location._id.toString());
    ...
    this.GetDb((err, db) => {
      ...
      read(db, 'user', userQuery, userFields, userOptions, (getUserError, users) => {
        ...
        read(db, 'location', locationQuery, locationFields, locationOptions, (locationReadError, locations) => {
          ...
          const stringLocations = (locations || []).map(location => {
            return {
              _id: location._id.toString(),
              userIds: (location.userIds || []).map(userId => userId.id.toString())
            };
          });
          const usersWithLocationIds = (users || []).map(user => {
            user.locationIds = stringLocations.filter(location => location.userIds.indexOf(user._id.toString()) > -1).map(p => p._id);
            return user;
          });
    ...
    this.GetDb((err, db) => {
      ...
      read(db, 'user', userQuery, userFields, userOptions, (getUserError, users) => {
        ...
        read(db, 'location', locationQuery, locationFields, locationOptions, (locationReadError, locations) => {
          ...
          const usersWithlocationIds = (users || []).map(user => {
            user.locationIds = locations.filter(location => location.userIds.find(userId => userId.id === user._id.toString())).map(p => p._id);
            return user;
          });
          return cb(null, usersWithlocationIds);
        });
