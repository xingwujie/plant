...
class MongoDb {
  ...
  getNotesWithPlants(notes, cb) {
    ...
    const myPlantIds = [];
    notes.map(({ plantIds }) => {
      plantIds.map((id) => { myPlantIds.push(id); return true; });

      return true;
    });

    this.getPlantsByIds(myPlantIds, null, (err, plants) => {
      if (err) return cb(err);

      // Map over the notes and add the plant info to them
      /* notes.map((currNote) => {
        currNote.plants = plants.filter((currPlant) => {
          //
          // Cast this to a string otherwise it will fail, str to obj comparison
          const allIds = currNote.plantIds.toString();

          return allIds.indexOf(currPlant._id) !== -1;
        });
      ...
      for (let i = 0; i < notesCopy.length; i + 1) {
        notesCopy[i].plants = plants.filter((currPlant) => {
          //
          // Cast this to a string otherwise it will fail, str to obj comparison
          const allIds = notesCopy[i].plantIds.toString();
